define(function() {
	...
	EJS.load = function(name, parentRequire, load, config) {
		...
		if(config.isBuild) {
			...
			built = shell.cat(filename).replace(cleanPath.replace(/\./g, '_'), name.replace(/\//g, "_") + '_ejs');
	...
	EJS.write = function (pluginName, name, write) {
		if (buildMap.hasOwnProperty(name)) {
			var text = buildMap[name],
				builtStr = 'define("' + pluginName + "!" + name + '", ["can/view/ejs"], function(can){ var __res =' + text + '; return function(data, helpers){ return can.view("' + name.replace(/\//g, '_') + '_ejs")(data, helpers); } })';
